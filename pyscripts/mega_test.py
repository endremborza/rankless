...
def _wait_http(url: str, timeout: int = 120, interval: float = 2.0) -> bool:
...
def _start_dev_server() -> subprocess.Popen:
    ...
    print(f"Dev server PID {proc.pid} started, waiting for {BASE_URL} …", flush=True)
    if not _wait_http(BASE_URL):
        proc.kill()
        sys.exit(f"Dev server did not come up within 120 s at {BASE_URL}")
