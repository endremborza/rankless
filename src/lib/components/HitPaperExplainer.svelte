	...
	<details class="hit-paper-explainer">
		...
		<p>
			...
			the paper's own subfield and year together ({formatShare(rule.wSfYear)}), its year alone ({formatShare(
				rule.wYear
			)}) and its subfield alone ({formatShare(rule.wSf)}). A paper needs at least {rule.minNeeded} citations,
			and then qualifies if <strong>any</strong> of the following hold:
		</p>
		<ul>
			...
			<li>it originates one of its research topics.</li>
		</ul>
		<p>
			A paper published no later than the year one of its authors won a Nobel prize counts
			{trimZeros(rule.nobelMultiplier)}× against the benchmark. Papers from the current year are not
			counted yet.
		</p>
	</details>
{/if}
