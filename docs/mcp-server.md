...
- `--investigate <run>[:<id>]` — deepen a past finding. Every finding has a stable `id`
  (`f1`, `f2`, …) in its run's `findings.json`; `--investigate live-demo:f5` loads that
...
- **`/mcp`** (`src/routes/(stat)/mcp/+page.svelte`, "Developers" in the footer) — docs:
  connect-your-agent snippets, the tool/foci/option reference, and a showcase. Fully static:
  it renders a baked manifest, `src/lib/assets/data/mcp-manifest.json`, generated by
...
- **`/mcp/runs`** — gallery of public, completed sessions. **`/mcp/runs/[name]`** — one
  session: its command, metadata, and outputs, rendered from `findings.json` via
  `SessionFindings.svelte` (raw `report.md`/`reproduce.md`/`findings.json` under `…/raw/<file>`).
- **`/admin/mcp`** (ORCID-gated via `isAdmin`) — list all sessions, toggle visibility, delete,
  and a form that **enqueues** a new run.
...
worker uses Python's `sqlite3` on the same WAL file.
...
`claude-sonnet-5`, model per-session), and ingests `findings.json` meta → `done`.
`make mcp-seed` populates the public gallery from existing writeups. deep.py's output root is
overridable via `--out-root` or `RANKLESS_WRITEUPS_DIR` (so personal PKM runs still land in
`.cril/`).

## Deployment (alpha)

The public MCP endpoint and the worker run as systemd `--user` services on the host; templates
are in `deploy/` (adjust paths/env, then `systemctl --user enable --now`):

- `deploy/rankless-mcp-server.service` — `mcp_server --transport streamable-http` on
  `MCP_PORT` (8100). `deploy/nginx-mcp.conf` adds a `location /mcp` proxy to the existing
  `alpha-api` server block, exposing `https://alpha-api.rankless.org/mcp`. Set
  `MCP_PUBLIC_URL` to that when baking the manifest.
- `deploy/rankless-mcp-worker.service` — the worker. **Requires an authenticated `claude` CLI
  in the service user's home** (`%h/.claude`); runs are sandboxed to `--allowedTools
mcp__rankless` (read-only citation tools, no bash/fs). `ADMIN_ORCIDS` gates who can create.
- Rate-limiting/keys for the public endpoint are still open (nginx/IP is the near-term lever;
  see `.cril/ideas.md` §8 Phase 2+).

Wiring these into `pyscripts/deploy.py` (a `setup_mcp_services` method + the nginx location) is
the remaining deploy-automation step.
