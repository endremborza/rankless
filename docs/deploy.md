...
Guards first: no uncommitted artifact files, `HEAD == origin/<branch>`, the deploy host's user DB holds users (a dev checkout carries only the mega_test fixture — shipping from it would seed production with fixture rows), and `pyscripts/migration_scripts/` is empty (see Code deploys). Then `new_large_alpha` (EC2 instance, full setup from nothing) which includes the user-DB handoff — live → local → new box, all user tables, unexpired auth sessions included (see [mcp-server.md](mcp-server.md) → Moving sessions between boxes); the new box's tables take this host's schema — plus an observability bootstrap: the `rankless-status.service` unit (the `/status` producer on :5566 that `live_monitoring.py` polls) and a detached `ops` tmux session (btop + user-unit journal tail) so later logins just `tmux attach -t ops`. The data root reaches the box through the same push definition the fleet uses (`manifest.push_data`: a `--delete` mirror of everything the digest covers — `user-ledger/` and the `stamp` included — then the tree cache seeded additively), so a box built by either path carries a matching stamp. Ends with smoke checks: frontend root, `/v1/specs`, a slice, one tree response, one MCP `initialize` against `<backend>/mcp` (the unit is up and its host guard admits the domain), the backend port owned by `rankless-server` itself (an `ssh -R` tunnel from another box would answer `/v1/specs` with that box's version — the check reads `ss -lntp`, not the HTTP answer), and the per-FE-worker memory table.
...
A frontend deploy (`Transper.update_fe`) pulls the branch, then runs every script under `pyscripts/migration_scripts/` with the box's own venv before the build — the schema is brought up to what the new code expects before that code serves a request; a box without a user DB yet skips the step. That directory is **empty in steady state**. Production code speaks only the current schema (no runtime patching, no version sniffing), so a change that strands an already-deployed DB or data dir ships a stdlib-only idempotent script there for exactly one deploy cycle: the running boxes apply it at their next code deploy, the author runs it once on the primary host's own DB (`uv run -m pyscripts.migration_scripts.<name>`), and the next commit deletes it — `ship-alpha` and `promote` refuse while any script remains, so no catch-up outlives a release. A fresh box never needs one: its tables are created from the primary host's DB at the handoff. Retrofitting something else onto a running box (a new unit, say) is a one-off command from a Python one-liner, not a primitive or make target. Then blue/green: build into the staging slot, restart + probe its workers, point nginx at it, flush the FE cache, stop the other slot. A data deploy (`update_data`) pulls, rebuilds the backend, pushes the data root with `manifest.push_data` and restarts the backend. Both end by syncing the box's venv and restarting the MCP server and worker, which run from the checkout — a data deploy after the backend restart, since the MCP server describes its tools from the running backend's `/v1/columns` when it starts.

### Hotfix on live while main is ahead

`sync_fe_to_live` pulls the box's branch, so once main carries commits that live's backend does not serve (a new endpoint, a changed response shape, a cache layout) it would build a frontend live cannot run. A frontend-only fix then ships from a branch off the live commit: read the box's HEAD (`git -C ~/rankless-deploy rev-parse HEAD` over ssh; the checkout is on `rankless-main`), `git checkout -b live-hotfix <that commit>`, cherry-pick the fix commit(s) from main, push, check the branch out on the box (`git -C ~/rankless-deploy checkout live-hotfix`, the one write outside the primitives, so it is confirmed like any other write to live; a stale local branch of that name has no upstream, so confirm `git status -sb` shows `...origin/live-hotfix` and set it if not), then `make sync_fe_to_live`: it pulls that branch, runs whatever `pyscripts/migration_scripts/` holds at that commit (empty in steady state), builds into the staging slot and flips; backend and data are untouched. The fix must apply at the live commit and must not need anything the live backend lacks, otherwise it is a promote, not a hotfix. The branch is deleted after the next promote, which rebuilds live from main.

## Front door (nginx)

`Transper.setup_nginx` renders the site conf through `render_nginx_conf` (`sync_nginx_to_{alpha,live}` re-renders and restarts; it is the `site` step of the ops definition). Both public hosts sit behind Cloudflare, so the conf trusts `CLOUDFLARE_RANGES` for `CF-Connecting-IP` and every per-client zone keys on the visitor, not the edge. The frontend host gets `pagelimit` (2 r/s, burst 20 per visitor), an in-flight cap on renders (`pageload` 96 for the whole box, `pageconn` 8 per visitor, answered 503), a 30 s `proxy_read_timeout`, retries only on a dead worker (`proxy_next_upstream error invalid_header`, two tries) and FE upstreams rendered with `max_fails=0` so a slow render never marks the pool dead; a `$junk_ua` map returns 429 to user-agent strings no browser has sent in a decade. Hashed build assets (`/_app/immutable/`) have their own location with no page limit, held in `fe-cache` by their immutable `Cache-Control`, so a box without an edge in front (the alpha) or an edge cache miss never spends a visitor's page budget on a page's chunk set. The API host and `/mcp` share `apilimit` (10 r/s, burst 50 per visitor). On the alpha, `LOADTEST_TOKEN` renders the `$lt_*` maps that exempt the load-test driver from every limit and cache (`make capacity`). Provisioning also installs a `tmpfiles.d` rule that ages the share-card cache (`/tmp/rankless-cards`, written by `card-raster.ts`) out at 7 days. Directives are asserted in `pyscripts/tests/test_deploy.py`; the rendered file passes `nginx -t` on 1.24 (mount it with a stub upstream file and a self-signed cert into `nginx:1.24-alpine`).
