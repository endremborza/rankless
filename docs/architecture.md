...
| `FeatureShowcase.svelte`                                                      | Homepage "Meet the new features" section: per-feature cards (Login, Hit-papers, All Works, Peers, Co-authors). Reads the baked `homepage-showcase.json` (zero backend calls on load); composes the three data previews below                                                                                                                                                   |
| `ShowcasePeers.svelte` / `ShowcaseCoauthors.svelte` / `ShowcaseExport.svelte` | Static, real-data previews for the showcase: per-field/per-year mini `BarChart`s; a ring-layout co-author mini network (no Cytoscape); a live citation-style + BibTeX export of one baked paper (`reference-format.ts`)                                                                                                                                                        |
...
| `homepage_showcase.py`                                                          | Bakes `src/lib/assets/data/homepage-showcase.json` (one featured scholar's peers/co-authors/sample paper) for the homepage showcase; run via `make homepage_showcase` against a live backend |
