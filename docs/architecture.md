...
| `src/metrics.rs` | The constants every public number is defined by, and the functions over them: `impact_score` (hit papers ÷ dampened-free `papers^IMPACT_BETA`), `field_score` over `dampened_size` = `(papers + cohort mean)^FIELD_SCORE_BETA`, and the two published rules as `const` values — the hit-paper rule (`HIT_RULE`, including the topic-creator path) and the work screen (`WORK_SCREEN`, whose year window and per-entity minimums come from `env_consts`), together `METHODOLOGY`, served at `/v1/methodology`; `steps/derive_links3` and `filter.rs` read their fields, they do not restate them. Also `Level` (one breakdown level of a tree: entity + citation side, the identity of a first-level profile). The anchors the scores are validated against, and their assertions, are the test beside them. The metric registry lives beside the columns it reads, in `rankless_trees/src/metrics.rs` |
...
| `src/steps/a2_init_atts.rs` | Init attributes (DOIs, ORCIDs, biblio, topics, locations); Levenshtein name dedup; Nobel category |
