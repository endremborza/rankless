...
| `(stat)/game-countries/`, `api/game-countries/`                                                          | "Name that country" sudden-death speed quiz over `country-card` objects (misleadingly named universities, 4 options, per-question timer); a freshly shuffled deck ships with the page, practice decks GET, runs POST into `country_game_results` |
...
| `GameShell.svelte`                                                                                         | Shared frame for the game pages: centered card, title + streak header, and the chrome classes the games share (`intro`/`mode-row`/`actions`/`reveal`/`verdict`/`error`)                                                                                                                                                                                                                                   |
...
| `lib/utils/game-countries.ts`     | Country-game rules: question timer, random capped deck build (answer folded into shuffled options), share text (types in `lib/types/game-countries.ts`)                                                                                                                                      |
| `lib/server/game-common.ts`       | Shared server plumbing for the games: lazy per-game schema setup, boundary validators, capped JSON body reader                                                                                                                                                                               |
...
| `lib/server/game-countries.ts`    | Server side of the country game: `country-card` pack reads, freshly shuffled decks, `country_game_results` run log + boundary validation                                                                                                                                                     |
