...
| `lib/server/objects.ts`           | Read/review access to the MCP object store (index rows + cached zstd bundle reads; written by pyscripts, see [mcp-server.md](mcp-server.md)): `/game-clues` and `/game-homeground` consume current cards, `/mcp` reviews and presents; SQL shared with the e2e seeder via `objects-schema.ts` |
