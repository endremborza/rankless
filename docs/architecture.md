...
| File | Role |
| --- | --- |
| `src/main.rs` | Allocator (mimalloc), module decls, `main`/`async_main`: `/v1` route table + socket bind; the static routers (`/counts`, `/specs` with the version stamp, `/methodology` = the work screen + the hit-paper rule, serialized once at startup) |
