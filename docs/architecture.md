...
| `src/metrics.rs` | The metric registry (`METRICS`: id, label, meaning text, parameters, kind per root type — global or intricate — and for a metric read off a tree's first level the `Level` it profiles, so the server withholds it where no tree yields that level) and the one size-adjusted score behind it: `dampened_size` = `(size + cohort mean)^SPEC_BETA`, `size_adjusted_score` = citations ÷ dampened papers (impact score with all citations, field score with one field's); the pipeline's dampened field divisor for author paper-field chips reads the same function |
...
| `src/handlers/` | Axum handlers by concern: `search` (names/sem-id/orcid/resolve), `entity` (views/trees/ladder/tops + meta), `peers`, `works` (paper sets + DAG + CNF work-set intersection), `table` (`/metrics` registry, `/slice` cohort pages ranked by a global metric with `sort=`/`subfield=`, the cohort size in `x-cohort-total`, `pin=` for named entities' rows in the active ordering, `/metrics/:etype?ids=` page-local metric values) |
...
**Metrics** are declared once in `rankless_rs/src/metrics.rs` and served at `/v1/metrics`; every metric is global or intricate per root type, and a metric with parameters declares a header template (`{param}`) the table fills with the chosen entity's name, so a column reads "Oncology score" or "Cited from Canada" rather than the generic label. A global metric is one number per entity for the whole cohort, so `/v1/slice/:etype/:from/:to?sort=` orders the cohort by it (citations, papers, impact score, and for authors h-index and career centroid); `subfield=` narrows the cohort to entities active in a field and adds field citations + field score, a per-request scan of the resident `cit_subfields` rows, so only for institutions, sources and countries; `pin=` returns the named entities' rows in the active ordering, rank-less outside a narrowed cohort. An intricate metric is answered for a page of ids by `/v1/metrics/:etype?ids=&metrics=` with the parameters in the query: year-windowed counts, an author's field citations/score, and the citing-country share — the chosen country's leaf of each entity's citing-country first-level profile over the root's links, one profile query per id fanned across the tree pool and read from the level-keyed cache; a metric that reads a profile declares its `Level`, and `/v1/metrics` withholds it for the root types whose trees never open with that level, so the registry cannot promise what no tree yields. Precomputing an intricate metric promotes it to global without touching the column model or the tool surface: the registry entry flips its kind, and the row shape (`TableRow` + `ROW_FIELD`) and the slice ordering each gain one entry.
