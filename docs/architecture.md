...
| `src/metrics.rs` | `Level` (one breakdown level of a tree: entity + citation side, the identity of a first-level profile) and the one size-adjusted score: `dampened_size` = `(size + cohort mean)^SPEC_BETA`, `size_adjusted_score` = citations ÷ dampened papers (impact score with all citations, field score with one field's); the pipeline's dampened field divisor for author paper-field chips reads the same function. The metric registry lives beside the columns it reads, in `rankless_trees/src/metrics.rs` |
