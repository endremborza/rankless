...
| `(stat)/game-countries/`, `api/game-countries/`                                                          | "Name that country" speed quiz over `country-card` objects (misleadingly named universities, 4 options, per-question timer, 3 lives — a miss holds the reveal, the last one ends the run); a freshly shuffled deck ships with the page, practice decks GET, runs POST into `country_game_results` (score, deck size, missed sem-ids) |
