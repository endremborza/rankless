...
| `src/state.rs` | The search side of a root type: `NameState` (the engine, the responses in citation order, `sem_to_dm` → dm id, `oa_to_rid`/`dm_to_rid` → response index, the `Orderings` — response ids sorted by papers, impact score, h-index and career centroid, built at startup from the columns), the per-entity reads over `RootColumns` a view needs (`serializable_ext`, `start_year`, `year_window`, `hit_papers`; hero relations + co-author network rebuilt per view from the mmapped tables), `IsTop`, type aliases (`StatesT`, `InstTrm`, `NameStateMap`) |
