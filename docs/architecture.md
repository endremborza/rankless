...
| File | Role |
| --- | --- |
| `src/main.rs` | Allocator (mimalloc), module decls, `main`/`async_main`: `/v1` route table + socket bind; the static routers (`/counts`, `/specs` with the version stamp, `/methodology` = the work screen, the paper-score constants and the methodology texts filled from them, `/columns` = the metric registry per root from `handlers::table::column_registry`, all serialized once at startup) |
