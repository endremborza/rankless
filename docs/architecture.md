...
| `(stat)/game/`, `api/game/`                                                                              | "Guess the institution" daily/practice game over `game-card` objects from the MCP object store; only the daily card ships with the page — practice cards load on demand (GET), results POST into `game_results` |
...
| `lib/utils/game.ts`               | Game math: map↔lat/lon projection, haversine, distance×clue scoring, daily card pick, streak rule, share text (types in `lib/types/game.ts`)                                                                                                                      |
| `lib/server/game.ts`              | Server side of the game: pack reads over the object store (`GAME_PACK_ETYPE` picks the served pack), daily/practice card picks with facts stripped, `game_results` log + boundary validation                                                                      |
| `lib/server/objects.ts`           | Read/review access to the MCP object store (index rows + cached zstd bundle reads; written by pyscripts, see [mcp-server.md](mcp-server.md)): `/game` consumes current cards, `/mcp` reviews and presents; SQL shared with the e2e seeder via `objects-schema.ts` |
...
| `backup.py`                                                                     | Off-box user-DB + artifact backups (`uv run -m pyscripts backup --source live\|alpha\|local`): hot DB snapshot → dated zstd + additive artifact-dir mirror under `<dest>/<source>/`; keeps last 7 daily + every 1st-of-month snapshot forever; daily timer via `services.py --backup-source` (see [deploy.md](deploy.md) → Backups)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
