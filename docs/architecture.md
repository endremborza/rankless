...
| `src/csv_writers.rs` | `to-csv`: flattens the snapshot's `data/jsonl/<entity>` gz JSON lines into per-entity zstd CSV parts under `entity-csvs/`; an entity with no `.gz` input is an error |
